...
contract TipJar{
    ...
    //1美元=0.0005ETH,汇率就是5*10^14    
    mapping(string => uint256) public conversionRates;
    mapping(address=>uint256) public tipPerPerson;
    string[]public  supportedCurrencies;//支持的货币清单
    ...
    function tipInCurrency(string memory _currencyCode, uint256 _amount)public payable{
        ...
        tipPerPerson[msg.sender] += msg.value;
        totalTipsReceived += msg.value;
        tipsPerCurrency[_currencyCode] += _amount;//和直接付ETH比起来是加amount的
        
    }

    function withdrawTips()public onlyOwner{
        uint256 contractBalance = address(this).balance;//把合约里的总余额拿出来，存到变量里
